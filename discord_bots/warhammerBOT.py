...
# command prefix that triggers the bot for a response

bot = commands.Bot(command_prefix='>', description='This bot will randomly generate your warhammer RP character and assign it to you!')

# This is the check to make sure the bot is logged in and ready to receive commands.
@bot.event
async def on_ready():
    print('Logged in as')
    print(bot.user.name)
...
# command to call warhammerRP.py code. Randomly Generate a Warhammer RP Character.
@bot.command()
async def warhammer(ctx):
    # imports for warhammerRP.py specifically
    import random
    import time

# Functions for creating all of the embedded prompts.
    def welcome():
        welcome = """
...
# fucntions that do the logic for warhammerRP.py
    def randomSpecies():
...
# variables storing the returns from the prompt functions.
    wl = welcome()
    sp = speciesPrompt()
    rc = classPrompt()
    att = attPrompt()
# variables storing the returns from the logic functions.
    species = randomSpecies()
    stDict = species[2]
    classCareer = randomClass(species)
    attTable = attributeTable(species)

# exit prompt function taking in all the returned logic and formatting it into a nice prompt.
    def exitPrompt(species, dict, classes, atts):
        exitP = f"""
...
# all the send calls to the discord channel.
    ...
    aembed.add_field(name="Part 0: Welcome Prompt", value=wl)
    bembed = discord.Embed(title="Warhammer Bot", color=0xeee657)
    bembed.add_field(name="Part 1: Species", value=sp)
    cembed = discord.Embed(title="Warhammer Bot", color=0xeee657)
    cembed.add_field(name="Part 2: Class and Careers", value=rc)
    dembed = discord.Embed(title="Warhammer Bot", color=0xeee657)
    dembed.add_field(name="Part 3: Attributes", value=att)
    eembed = discord.Embed(title="Warhammer Bot", color=0xeee657)
    eembed.add_field(name="Part 4: attributes", value=exitPT)
